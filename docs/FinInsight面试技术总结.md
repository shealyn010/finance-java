...
传统做法是固定发送最近 N 条消息给 LLM，但 N 太小会丢失上下文（"那它还剩多少？" 不知道 "它" 是谁），N 太大 token 成本爆炸且超过 LLM 上下文窗口。
...
完整对话 (100+ 轮)
   │
   ├── 工作记忆 (最近5轮)  → 完整原文 → 占用 ~500 token
   ├── 短期记忆 (5-30轮)    → LLM 摘要压缩一句话 → 占用 ~200 token  
   └── 长期记忆 (跨会话)    → 向量库 Chroma 检索 → 按需注入 top3 chunk
```

**关键数字**：
- 最近5轮完整对话: 精确理解代词指代和紧邻上下文
- 5轮前压缩摘要: 为"面试追问"场景保留前文要点，不丢关键信息
- 跨会话向量检索: 下周打开还能记得上次聊了什么（待接 Chroma）

**面试讲法**：
"对话记忆不能简单用固定条数——10条在多轮追问场景不够，100条 token 成本太高。我设计了三级记忆：工作记忆保留最近5轮完整对话确保代词理解，短期记忆用 LLM 对更早对话做摘要压缩控制 token，长期记忆通 过向量库做跨会话检索。这个设计在追问场景中效果显著。"
